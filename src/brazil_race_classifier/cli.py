...
from . import __version__, TSE_CONFIG, PROJECT_CONFIG
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument("--config", default=TSE_CONFIG, help="Path to YAML with candidates URLs")
    ...
    # 1) Download TSE candidate data and upload to GCS
    return run_download_candidates(config=args.config,
                                   bucket=args.bucket,
                                   project=args.project,
                                   years=args.years)

    # 2) Download TSE photos and upload to GCS
